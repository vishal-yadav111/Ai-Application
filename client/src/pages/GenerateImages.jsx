      ...
      <form onSubmit={onSubmitHandler}className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'> 
          {ImageStyle.map((items, index)=> (
            <span onClick={() => setSelectedStyle(items)} className={`text-xs px-4 py-1 border rounded-full
               cursor-pointer ${selectedStyle === items ? 'bg-green-50 text-green-700' :
              'text-gray-500 border-gray-300'}`} key={items}>{items}
            </span>

          ))}
        </div>
          <div className='my-6 flex items-center gap-2'>
                 <label className='relative cursor-pointer w-9 h-5'>
                 <input
                  type='checkbox'
        ...
        bg-gradient-to-r bg-[#0f172a] text-white px-4 py-2 mt-6
