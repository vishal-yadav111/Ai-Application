      ...
      <form onSubmit={onSubmitHandler}className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'> 
          {articleLength.map((items, index)=> (
            <span onClick={() => setSelectedLength(items)} className={`text-xs px-4 py-1 border rounded-full
               cursor-pointer ${selectedLength.text === items.text ? 'bg-blue-50 text-blue-700' :
              'text-gray-500 border-gray-300'}`} key={index}>{items.text}
            </span>
